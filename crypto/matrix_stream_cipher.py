...
# ---------------- gamma core (f64, LFSR, multiplicative triple) ----------------
...
def _f64_triple(m_word: int, g_prev: int, f_prev: int, k_curr: int, k_prev: int) -> int:
    """
    3-argument variant for the stream cipher with multiplicative-only update
    and protection against zero operands/results.

    Effective operands:
      - M_eff = M_i, if M_i != 0; else M_eff = K_i
      - g_base = g_{i-1}, if g_{i-1} != 0; else g_base = K_{i-1}

    Then:
        g_i = ((M_eff * g_base * F_{i-1})_L
               + (M_eff * g_base * F_{i-1})_R) mod 2^64

    If result is 0, it is replaced by K_{i-1} to avoid zero gamma.
    """
    m_eff = m_word if m_word != 0 else (k_curr & MASK64)
    g_base = g_prev if g_prev != 0 else (k_prev & MASK64)

    prod = (m_eff & MASK64) * (g_base & MASK64) * (f_prev & MASK64)
    lo = prod & MASK64
    hi = (prod >> 64) & MASK64
    g = (hi + lo) & MASK64
...
def gamma_chain_no_pad(
    ...
    g0 = K0. For each 64-bit word X (first words of M0, then words of the rest):
      - F and K evolve via independent 64-bit LFSR streams seeded from K0.
      - g is updated by pure multiplicative f64_triple with non-zero
        protection for M and g using the current (K_i) and previous (K_{i-1})
        key values.
      - if g == 0 and salt is provided, salt is added modulo 2^64
        (theoretically redundant, but kept for compatibility).
    ...
    g = K0 & MASK64
    f_state = K0 & MASK64
    k_state = K0 & MASK64  # K_0
    k_prev = k_state       # K_{-1} := K_0 by convention
    ...
    for idx, x in enumerate(words):
        ...
        k_curr = k_state      # K_i
        k_prev_for_g = k_prev # K_{i-1} (for i=0 it's also K_0)

        g = _f64_triple(x & MASK64, g, f_state, k_curr, k_prev_for_g)
...
def encrypt(text: str, K0: int, Gamma: List[List[int]]) -> Ciphertext:
    ...
    Steps:
      ...
      • MAC chain: g0 = K0; process M0 words, then rest using multiplicative
        gamma update with LFSR-driven F and K streams.
      • Stream XOR only for the *rest*:
            C_i = X_i XOR g_{i-1},
        where g_{i-1} evolves via the same multiplicative rule with LFSR(F,K).
    ...
    # starting gamma for stream encryption (after 3 M0 words)
    g_prev = g_trace[3] if len(g_trace) >= 4 else g_trace[-1]
    X_words = pack_u64_stream_no_pad(rest)

    # LFSR states after processing the three M0 words
    f_state = K0 & MASK64
    ...
    for x in X_words:
        ...
        f_state = lfsr_step(f_state)
        k_curr = k_state
        g_prev = _f64_triple(x & MASK64, g_prev, f_state, k_curr, k_prev)
...
def decrypt(ct: Ciphertext, K0: int, Gamma: List[List[int]]) -> str:
    """
    Decrypt and verify MAC according to the same no-pad, multiplicative rules.
    """
    ...
    # LFSR states after three M0 words
    f_state = K0 & MASK64
    for _ in range(len(m0_words_no_pad(m0_bytes_18))):
        f_state = lfsr_step(f_state)
    ...
    for c in ct.stream:
        ...
        f_state = lfsr_step(f_state)
        k_curr = k_state
        g_prev = _f64_triple(x, g_prev, f_state, k_curr, k_prev)
    ...
    if (mac_check & MASK64) != (ct.mac & MASK64):
        raise ValueError(
            f"MAC mismatch: calc={mac_check:#018x} vs recv={ct.mac:#018x}"
        )
